...
| タイマー   | 選択した時間（10秒/30秒/60秒）のカウントダウン | 必須   |
...
| 機能         | 説明                    |
| ------------ | ----------------------- |
| ChatGPT API連携 | OpenAI APIから出題文を動的生成（ローカル/APIを選択可能） |
| 制限時間選択 | ゲーム開始前に 10秒/30秒/60秒 から選択可能（デフォルト: 30秒） |
...
- 選択した時間（デフォルト30秒）の対戦が正常に完了し、勝敗が表示される
- 日本語文章をローマ字入力で打てる
...
- 10秒/30秒/60秒から選択できるカウントダウンタイマー（デフォルト: 30秒）
- リアルタイムでスコアを更新・表示
